class SidewaysPlat extends Phaser.Physics.Arcade.Sprite {
    constructor(scene, x, y, endX, velocity, texture) {
    ...
    update(){
        // 32 is half our tile width
        this.setVelocityX(this.velocity);
        if(this.x < this.end && (this.currTime - this.time) > 2000){
            //console.log(this.currTime - this.time);
            this.velocity = -(this.velocity);
            this.time = this.currTime;
        } else if (this.x > this.start && (this.currTime - this.time) > 2000){
            //console.log(this.currTime - this.time);
            this.time = this.currTime;
            this.velocity = -(this.velocity);
        } else {
